...
- **抗噪重训判定（2026-09-13，M3 负结果）**：链整形逐拍噪声增强候选 v3-B 三次迭代均未过双门槛（带噪语料全面更优，但干净 test 事件 F1 退化 -0.15~-0.23）——**保留 v3-A**（TUNING_HISTORY §107，`v3b_iteration_summary.json`）。M4 探索结论：v3-A 的主要短板是**域偏移**（LUDB 干净正常 93% 误报）而非合成噪声（被 M1 状态机吸收）；SQI 门控被数据否定（FP 源 SQI 0.66-0.99 无区分力）。结论页与图：`research/M4_结论.md`（§108）。
