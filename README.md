...
## 当前状态（2026-09-19，R21 模型线专项）

- **R21 KD 锚定双头联合训练（2026-09-19，负结果轮，保留 v3-A）**：对症 §118/§119 三线图谱缺失件的预注册轮——骨干可塑（保泛化）+ abn 头对冻结 v3-A 的概率 KL 蒸馏锚（保 θ=0.5 校准）。**前置 W0**：语料 PTB test 域扩容 seg41..46（6 患者：3 MI + 心律失常/心肌病/束支阻滞，既有 31+7 case 逐字节不变，固件 corpus3 + idf.py build 过）；v3-A 在扩容组基线实测组均 0.754/单 case 最低 0.36——TP 判据去单点化的同时把判据改为对同 seed 基线配对。**H7 两迭代（λ_kd=1.0/3.0）均未过线关闭**：KD 锚可靠钉住判别力（6 次训练 AUC 全保 ±0.03），锚距离与语料 FP 漂移单调相关（可迁移诊断），但事件级 evF1 校准漂移不随 λ_kd 单调受控、种子方差主导；pure_motion gated 0.00×6（机制累计 27/27）。设计空间第四点位闭环：**双头联合训练方向整体关闭**。H8/H9 按条件规则未启动；板上模型/固件运行行为/App 零改动（TUNING_HISTORY §120）。
