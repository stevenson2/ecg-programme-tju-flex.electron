...
# -*- coding: utf-8 -*-
"""
eval_exp7b.py — P0-2 exp7b (lr warmup 重训) 修正后因果链口径评估
================================================================================
评估 exp7b (best_resnet_large_exp7b.h5) 在"修正后因果链"测试拍上的 MIT/PTB AUC,
与 exp6-SGD 在同口径下对照, 复核:
  1. MIT 是否回补 (对照 exp6-SGD 因果链 0.9090, TH 预测 ~0.90);
  2. PTB 是否保持改善 (对照 exp6-SGD 因果链 0.7621 / 历史 D3 0.7697)。

产出: models/deploy_match/retrain_exp7b_eval.json
      (沿用 retrain_exp7_eval.json 的 results/verdicts 结构)

锚点 (不可篡改, 来自 docs/FINAL_RESULTS.md + retrain_exp7_eval.json):
  exp6-SGD 部署链(D3)   : MIT 0.9122 / PTB 0.7697   (历史, FINAL_RESULTS 表4)
  exp6-SGD 修正因果链    : MIT 0.9090 / PTB 0.7621   (重测未重训, retrain_exp7_eval)
  exp6 患者级清洁 D0     : MIT 0.8942 / PTB 0.8232   (filtfilt 训练链上限)
  exp7 修正因果链 (有假象): MIT 0.8590 / PTB 0.7829   (第一步, epoch-0 欠训练)
"""
import argparse
import json
import sys
...
OUT_JSON = CACHE_DIR / "retrain_exp7b_eval.json"

# 复用 eval_exp7.py 已构建的修正后因果链测试拍缓存 (同链同拍, 无泄漏)
...
# 锚点 (docs/FINAL_RESULTS.md 表2/表4 + retrain_exp7_eval.json)
ANCHORS = {
    "mit": {
        "exp6_sgd_d3": 0.9122,       # 历史 D3 链 (无因果 HP)
        "exp6_sgd_causal": 0.9090,   # 重测修正因果链 (未重训)
        "exp7_causal": 0.8590,       # 第一步 exp7 (epoch-0 欠训练假象)
        "d0_target": 0.8942,         # filtfilt 训练链上限 (患者级清洁)
    },
    "ptb": {
        "exp6_sgd_d3": 0.7697,
        "exp6_sgd_causal": 0.7621,
        "exp7_causal": 0.7829,
        "d0_target": 0.8232,
    },
}
...
# ============================================================
# 构建/复用修正后因果链测试拍 (与 eval_exp7.py 完全一致, 缓存复用)
# ============================================================
...
def main():
    ...
    print("=" * 70)
    print("eval_exp7b.py — exp7b (lr warmup) 修正后因果链口径评估")
    print("=" * 70)

    # ---- 构建/加载修正后因果链测试拍 (复用 eval_exp7.py 缓存) ----
    t0 = time.time()
    ...
    # ---- 汇总 verdict (对照 0.9090 / 0.7621 / 0.7697 净收益) ----
    verdicts = {}
    if "exp7b" in results and "exp6-SGD" in results:
        for dom in ["mit", "ptb"]:
            eb = results["exp7b"][dom]["auc"]
            e6 = results["exp6-SGD"][dom]["auc"]
            causal = ANCHORS[dom]["exp6_sgd_causal"]
            d3 = ANCHORS[dom]["exp6_sgd_d3"]
            d0 = ANCHORS[dom]["d0_target"]
            e7 = ANCHORS[dom]["exp7_causal"]
            lines = [
                f"{dom.upper()} exp7b 因果链 AUC={eb:.4f} "
                f"vs exp6-SGD 因果链={e6:.4f} (Δ={eb-e6:+.4f})",
                f"{dom.upper()} exp7b 因果链 AUC={eb:.4f} "
                f"vs exp6-SGD 因果链锚点={causal:.4f} (Δ={eb-causal:+.4f})",
                f"{dom.upper()} exp7b 因果链 AUC={eb:.4f} "
                f"vs exp6-SGD D3={d3:.4f} (Δ={eb-d3:+.4f})",
                f"{dom.upper()} exp7b 因果链 AUC={eb:.4f} "
                f"vs exp7(有假象)={e7:.4f} (Δ={eb-e7:+.4f})",
                f"{dom.upper()} exp7b 因果链 AUC={eb:.4f} "
                f"vs D0 上限={d0:.4f} (残余失配={eb-d0:+.4f})",
            ]
            verdicts[dom] = lines
            print("\n  " + "\n  ".join(lines))
    ...
    output = {
        "meta": {
            ...
            "purpose": "P0-2 exp7b (lr warmup 重训) 修正后因果链口径评估 + 净收益复核",
            "chain": "corrected_deployment_chain = D3 部署链 + 因果 HP 0.5Hz (butter(2,0.5,fs=250))",
            "models": [m for m, _ in EVAL_MODELS],
            "thresholds": THRESHOLDS,
            "anchors": ANCHORS,
            "note": "exp7b 与 exp6-SGD 在同一修正后因果链测试拍上评估; exp6_sgd_causal 锚点为 "
                    "exp6-SGD 在该链上重测值 (未重训), exp6_sgd_d3 为历史 D3 链值, "
                    "exp7_causal 为第一步 (epoch-0 欠训练假象) 值。",
        },
        "results": results,
