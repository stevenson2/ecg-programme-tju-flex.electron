#!/bin/bash
# run_exp4_patient_clean.sh — exp4 (患者级清洁) 复现脚本 (T3-7/H3)
# exp4: PTB 训练拍 cap 100,000 (全量), 无域平衡; AdamW lr 5e-4; seed42 患者级
set -e
...
# 训练链口径 (filtfilt) + 患者级划分 + PTB cap 100000 (exp4 配置)
nohup python3 train.py --resnet-large --incart --ptb-beat --ptb-abn-max 100000 --patient-split --epochs 200 --patience 20 > ~/exp4_patient_clean_train.log 2>&1 &
echo "TRAIN_PID=$!"
