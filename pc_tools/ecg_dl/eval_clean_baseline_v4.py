#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""eval_clean_baseline_v4.py — v4 硬负例模型的患者级测试集评估 (TH §103)
================================================================================
与 §99/§100 完全同口径: θ=0.5、1-of-5、cooldown=5、同一测试缓存 npz。
输出: models/deploy_match/clean_baseline_eval_v4.json
"""
import json
import sys
import time
...
OUT = CACHE / "clean_baseline_eval_v4.json"
...
def main():
    ...
    for domain, labels, probs, rids in (
        ...
        print(f"[EVAL4] {domain}: AUC={beat['auc']:.4f} F1={beat['f1']:.4f} "
    ...
    report = {
        ...
        "purpose": "TH §103 clean_baseline_v4 (硬负例挖掘) 患者级测试集评估; "
                   "模型未见任何测试数据, 挖掘仅限训练患者",
    ...
    OUT.write_text(json.dumps(report, indent=2, ensure_ascii=False),
                   encoding="utf-8")
    print(f"[EVAL4] saved {OUT} ({time.time() - t0:.0f}s)", flush=True)
