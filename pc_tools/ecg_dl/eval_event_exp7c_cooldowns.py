#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""eval_event_exp7c_cooldowns.py — v3b θ=0.85 下扫描 alert_cooldown
================================================================================
...
OUT = Path(__file__).resolve().parent / "models" / "deploy_match" / "event_exp7c_cooldown.json"
MODEL = MODELS_DIR / "ecg_model_exp7c_int8.tflite"
...
def main():
    ...
    np.save(Path(__file__).resolve().parent / "models" / "deploy_match" / "exp7c_causal_probs_full.npy", probs)
    ...
    for cool in [3, 4, 5, 6, 7, 8, 10]:
        ...
        r = [x for x in rows if abs(x["theta"] - 0.50) < 1e-9 and x["policy"] == "1-of-5"]
        if r:
            row = r[0]
            results.append({
                "alert_cooldown": cool,
