#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""build_v6a5_dataset.py -- v6a5 small-dose clean hard-normal dataset.

Dataset = original v3-A train (5,942 beats) + 10,000 clean hard-normal beats
(score>0.95, completeness passed, INCART excludes I03/I73) sampled
...
OUT = CACHE / "v6a5_train_10k_hardnorm_anchor.npz"
SEED = 42
QUOTAS = {0: 2000, 1: 3000, 2: 19, 3: 4981}
...
def main():
    ...
    for code, quota in QUOTAS.items():
        ...
        if n_use <= 0:
            print(f"[V6A5] source {code}: quota={quota} available={len(cand)} -> 0")
            continue
        pick = rng.choice(cand, n_use, replace=False)
        selected_global.append(pick)
        print(f"[V6A5] source {code}: quota={quota} available={len(cand)} -> {n_use}")
    ...
    for code in sorted(set(src[sel].tolist())):
        ...
        # record id can be retrieved from source-specific arrays; for audit, use local as rec placeholder.
    ...
    record = np.concatenate([np.zeros(len(x_hard), dtype=np.int32),
    ...
    print(f"[V6A5] saved {OUT.name}: x={x.shape} n={len(y)} "
