#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""qat_exp7c_v3.py — exp7c_ecgfounder_v3 INT8 QAT
================================================================================
目的：
...
流程：
  ...
  4. 导出 INT8 TFLite: models/ecg_model_exp7c_ecgfounder_v3_qat_int8.tflite
"""
import sys, os, time
...
OUT_TFLITE = MODELS / "ecg_model_exp7c_ecgfounder_v3_qat_int8.tflite"
OUT_JSON = MODELS / "qat_exp7c_v3.json"
SEED = 42
...
def main():
    ...
    ptb_a, ptb_al, ptb_n, ptb_nl = load_domain("ptb", 800, 200)
    ...
    x_train = np.concatenate([x_mix, x_real, x_pubnorm])
    y_train = np.concatenate([y_mix, y_real, y_pubnorm])
    ...
    result = {
        ...
        "data": {"train": int(len(x_train)), "real": int(len(real)), "pub_normal": int(len(pubnorm)), "epochs": int(len(hist.epoch))},
