#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""eval_event_v3b_vs_exp7c.py — exp7c INT8 vs ECGFounder v3b QAT INT8 事件级 1-of-5
================================================================================
...
OUT = Path(__file__).resolve().parent / "models" / "deploy_match" / "event_v3b_vs_exp7c.json"

MODELS = [
    ("exp7c_int8", MODELS_DIR / "ecg_model_exp7c_int8.tflite"),
    ("v3b_qat_int8", MODELS_DIR / "ecg_model_exp7c_ecgfounder_v3b_qat_int8.tflite"),
]
...
def main():
    ...
    for tag, path in MODELS:
        ...
        rows = evaluate_sequence_set("test", rr, yy, pp[te_red], ss,
                                     DEFAULT_GT_GAP, DEFAULT_ALERT_COOLDOWN)
        # 提取 1-of-5 且 theta=0.50 的行（如无则取第一个 1-of-5）
        chosen = [r for r in rows if r["policy"] == "1-of-5" and abs(r["theta"] - 0.5) < 1e-9]
        if not chosen:
            chosen = [r for r in rows if r["policy"] == "1-of-5"][:1]
        results[tag] = chosen
        for r in chosen:
