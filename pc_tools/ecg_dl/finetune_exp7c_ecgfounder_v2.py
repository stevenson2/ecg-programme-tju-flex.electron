...
# -*- coding: utf-8 -*-
"""
finetune_exp7c_ecgfounder_v2.py — exp7c + ECGFounder 代理硬负样本弱标签微调 v2 (低权重)
================================================================================
在 exp7c 基础上，加入：
...
输出：
  models/best_resnet_large_exp7c_ecgfounder_v2.h5
  models/train_history_exp7c_ecgfounder_v2.csv
  models/deploy_match/finetune_exp7c_ecgfounder_v2.json
"""
...
OUT_H5 = MODELS / "best_resnet_large_exp7c_ecgfounder_v2.h5"
OUT_CSV = MODELS / "train_history_exp7c_ecgfounder_v2.csv"
OUT_JSON = CACHE / "finetune_exp7c_ecgfounder_v2.json"
...
def main():
    ...
    # ECGFounder 硬负样本（弱标签 abnormal），只取前 HARDNEG_TOP_K 个记录
    all_hardneg = np.load(EGF_DIR / "hardneg_beats.npy").astype(np.float32)
    hard_meta = json.load(open(EGF_DIR / "hardneg_beats_meta.json", encoding="utf-8"))
    n_sel = 0
    idx_list = []
    for rec in hard_meta["records"][:HARDNEG_TOP_K]:
        n_sel += rec["n_beats"]
        idx_list.append(rec["n_beats"])
    hardneg = all_hardneg[:n_sel]
    print(f"[FT-EGF] real normal={len(real)}, ecgfounder weak abn={len(hardneg)} (top {HARDNEG_TOP_K} records)")
    ...
    # 训练集（打乱）+ 样本权重：真实正常权重最高，ECGFounder 弱异常权重大幅降低
    ...
    w_train = np.concatenate([
        ...
        np.full(len(x_rtr), REAL_WEIGHT, dtype=np.float32),
        np.full(len(x_hard), HARDNEG_WEIGHT, dtype=np.float32),
    ])
    ...
    print(f"[FT-EGF] train={len(x_train)}, val={len(x_val)} "
          ...
          f"hardneg_weight={HARDNEG_WEIGHT}, real_weight={REAL_WEIGHT}")
    ...
    result = {
        ...
        "config": {
            ...
            "optimizer": "adam",
            "loss": "sparse CE",
            "class_weight": None, "sample_weight": {"real_normal": REAL_WEIGHT, "ecgfounder_weak_abn": HARDNEG_WEIGHT},
