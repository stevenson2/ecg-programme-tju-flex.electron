#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""qat_exp7c.py — exp7c INT8 量化感知训练（QAT）
================================================================================
目的：
...
流程：
  1. 加载 best_resnet_large_exp7c.h5
  2. tfmot.quantization.keras.quantize_model
  3. 用因果部署链混合数据 + 真实正常拍微调少量 epoch
  4. 导出 INT8 TFLite: models/ecg_model_exp7c_qat_int8.tflite
"""
import sys, os, time
...
OUT_TFLITE = MODELS / "ecg_model_exp7c_qat_int8.tflite"
OUT_JSON = MODELS / "qat_exp7c.json"
SEED = 42
...
def main():
    ...
    mit_a, mit_al, mit_n, mit_nl = load_domain("mit_bih", 1000, 300)
    ...
    x_train = np.concatenate([x_mix, x_real])
    y_train = np.concatenate([y_mix, y_real])
    ...
    hist = qat_model.fit(
        ...
        batch_size=32, epochs=8, verbose=2,
    )
    ...
    result = {
        ...
        "purpose": "exp7c QAT -> INT8",
        "data": {"train": int(len(x_train)), "real": int(len(real)), "epochs": int(len(hist.epoch))},
