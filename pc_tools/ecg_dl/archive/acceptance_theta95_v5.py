#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""acceptance_theta95_v5.py — v5 冻结操作点完整验收门 (TH §104)
================================================================================
与 acceptance_theta95_v4.py (§103) 完全同门口径, 仅换模型为
clean_baseline_v5; 操作点冻结 (θ=0.95 / 1-of-5 / cooldown=20, 不重扫):

  阈值相关门:
    - 拍级混淆矩阵 = 原始概率 > 0.95
    - 事件级 / FP/rec / 报警块 = 策略 (0.95, 1-of-5, cooldown=20)
  阈值无关门:
    - clean AUC (两域)、真 AFE 留出、训练 best val AUC (读 v5 训练 JSON)

测试集在训练/扩训/调参全程零接触, 本脚本是唯一评估入口。
输出: models/deploy_match/acceptance_theta95_v5.json
"""
import json
import sys
import time
...
TRAIN_JSON = CACHE / "train_clean_baseline_v5.json"
OUT = CACHE / "acceptance_theta95_v5.json"
...
def main():
    ...
    report = {
        ...
        "purpose": "TH §104: clean_baseline_v5 (SVDB 扩训) 在冻结操作点 "
                   "(θ=0.95, 1-of-5, cooldown=20) 上的完整验收门; 操作点继承 "
                   "§101/§102, 不重扫; 测试集从未参与训练/扩训/选择",
    ...
    OUT.write_text(json.dumps(report, indent=2, ensure_ascii=False),
                   encoding="utf-8")
    for k, (ok, v, c) in checks.items():
        print(f"[ACC5] {'PASS' if ok else 'FAIL'} {k}: {v} ({c})", flush=True)
    print(f"[ACC5] overall={report['verdict']['overall']} "
