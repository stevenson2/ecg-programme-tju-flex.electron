#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""acceptance_theta95_v3.py — θ=0.95 操作点上的完整验收门 (TH §102)
================================================================================
§101 在 val 上选中 θ=0.95 / 1-of-5 / cooldown=20。本脚本把 §100 的验收门
逐条搬到该操作点重算 (测试集从未参与选择):

  阈值相关门 (新操作点口径):
    - 拍级混淆矩阵 = 原始概率 > 0.95 (与 §100 的 θ=0.5 拍级口径同源平移)
    - 事件级 / FP/rec / 报警块 = 策略 (0.95, 1-of-5, cooldown=20),
      池化语义与 evaluate_sequence_set 一致 (复用 sweep_val_policy_v3)
  阈值无关门 (沿用模型固有量):
    - clean AUC (两域)、真 AFE 留出、训练 best val AUC

输出: models/deploy_match/acceptance_theta95_v3.json
"""
import json
import sys
import time
...
TRAIN_JSON = CACHE / "train_clean_baseline_v3.json"
OUT = CACHE / "acceptance_theta95_v3.json"
...
def main():
    ...
    report = {
        ...
        "purpose": "TH §102: §101 选中操作点 (θ=0.95, 1-of-5, cooldown=20) 上的"
                   "完整验收门; 测试集从未参与选择",
        "model": str(MODEL.relative_to(BASE)),
        "operating_point": POLICY,
        "metric_caliber": {
            "beat": f"原始概率 > {THETA} 的混淆矩阵 (与 §100 θ=0.5 拍级口径同源平移)",
    ...
    OUT.write_text(json.dumps(report, indent=2, ensure_ascii=False),
                   encoding="utf-8")
    for k, (ok, v, c) in checks.items():
        print(f"[ACC] {'PASS' if ok else 'FAIL'} {k}: {v} ({c})", flush=True)
    print(f"[ACC] overall={report['verdict']['overall']} "
