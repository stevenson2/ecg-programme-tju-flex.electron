#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""qat_exp7c_v4.py — exp7c_ecgfounder_v3 INT8 QAT + v3b hard-normal
================================================================================
目的：
...
流程：
  ...
  4. 导出 INT8 TFLite: models/ecg_model_exp7c_ecgfounder_v4_qat_int8.tflite
"""
import sys, os, time
...
OUT_TFLITE = MODELS / "ecg_model_exp7c_ecgfounder_v4_qat_int8.tflite"
OUT_JSON = MODELS / "qat_exp7c_v4.json"
SEED = 42
...
def main():
    ...
    hardnorm = np.load(MODELS / "ecgfounder" / "hard_normal_beats_v3b.npy").astype(np.float32)
    ...
    w_train = np.concatenate([
        ...
        np.full(len(hardnorm), 5.0, dtype=np.float32),
    ])
