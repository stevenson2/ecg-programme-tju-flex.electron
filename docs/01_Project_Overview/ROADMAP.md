...
| # | 任务 | 说明 |
|---|---|---|
| 1 | 关卡模型训练 | 当前最高优先级，直接决定双专家方案是否成立 |
| 2 | 级联 PC 模拟 | 输出决策门槛所需指标 |
| 3 | 论文补证 | 把 P1 补证、exp7c、部署链结果同步进稿件 |
| 4 | 固件双模型集成 | 单解释器分时复用，或按需切换模型 |
| 5 | 全链路验证 | PC-ESP32 一致性、真实采集、温度/功耗 |
| 6 | 人体实验 | 依赖伦理审批，目前阻塞 |
| 7 | 投稿准备 | 作者信息、模板转换、图表导出、润色 |
